...
public class ManageProductTest extends BaseClass {
	...
	public void verifyIfUserCanAddNewManagrProductWithoutMandatoryFields() throws IOException, AWTException {
		...
		manageproduct=homepage.clickOnManageProduct().clickOnNewManageProductButton().enterProductTitle().clickOnproductTypeRadiobutton()
				.enterTagOnTagFiled().selectGroupDropDown().selectPriceTypeRadioButton().enterValueOnWeightValue()
				.selectWeightUnit().enterMaximimQuantityCanOrder().enterPriceOnPriceField().enterMrpOnFiled().enterStockAvailability()
				.enterPurchasePriceValue().selectUnlimitedChechBox().selectStockRaduiButton().uploadImage().uplaodSubImages()
				.selectFeaturedRadioButton().selectComboPackRadioButton().clickOnSaveButton();
